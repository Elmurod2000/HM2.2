...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Toyota[] toyotas = {createObject("Camry"),createObject("LandCruiser"),createObject("Jeep")};
        for (int i = 0; i < toyotas.length; i++) {
            toyotas[i].print();
        }
        ...
        // который умеет создавать объекты класса 2й, 3й и 4й и после создания и задания свойств объекту метод возвращает ссылку
        // на объект (пульт). Можно использовать switch для того чтоб определить какого типа нужно создать экземпляр объекта.
    ...
    public static Toyota createObject(String className) {
        switch (className) {
            case "Camry":
                Camry camry = new Camry("Black","70");
                return camry;
            case "LandCruiser":
                LandCruiser landCruiser = new LandCruiser("Yellow","200");
                return landCruiser;
            case "Jeep":
                Jeep jeep = new Jeep("Черный","ДАД");
                return jeep;
        }
        return null;
    }
